...
class DownloadManager:
        ...
        try:
            while not self.stop_requested:
                ...
                try:
                    ...
                    print(f"平均速度: {self.format_bytes(self.download_progress['downloaded'] / download_time)}/s")
    ...
    def update_progress(self, chunk_size):
        ...
        if self.download_progress["total"] > 0:
            self.download_progress["percent"] = min(
                100,
                self.download_progress["downloaded"] / self.download_progress["total"] * 100
            )
    ...
    def display_progress(self):
        ...
        # 清除当前行并输出
        print(progress_info.ljust(shutil.get_terminal_size().columns - 1), end='', flush=True)
